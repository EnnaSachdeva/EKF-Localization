...
robot.controlCovariance = diag([0.5, 0.02]);
...
actualPose.theta=normalize_angle(actualPose.theta);
...
%     actualControl= sane_mvnrnd(idealControl',robot.controlCovariance,alphaControl,1);
    ...
    predPose.theta=normalize_angle(predPose.theta);
...
%     actualPose.theta=normalize_angle(actualPose.theta); 
    actualPose=runMotionModel(actualPose, actualControl); 
    actualPose.theta=normalize_angle(actualPose.theta); 
    
%     position{m}=[newactualPose.x,newactualPose.y]; 
    % TODO: Compute the jacobian of the motion model, to linearize it
       
    F = computeJacobianState(updatePose, idealControl);    %%%%% actualPose at time t
...
%     sensor_measurements-pred_measurements
...
%      sensor_measurements
   ...
   for p=1:size(sensor_measurements,1)  
        H(((2*p)-1):2*p,1:size(robot.covariance,1))=computeJacobianObs(predPose,pred_measurements, params.map);   %%%% 2Nx3 matrix
...
%         inc= K*inn_vect;
%        inn_vect
       robot.pose.x = predPose.x+ inc(1);
       robot.pose.y = predPose.y+ inc(2);
       robot.pose.theta = predPose.theta+ inc(3);
       robot.pose.theta=normalize_angle(robot.pose.theta);
